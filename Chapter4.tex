...
The title of the section indicates a class from the OBI ontology. A study design execution is a part of an investigation and consist of several different assays. An assay is a process for assessing a certain quality of an input object. In our case the input objects are bone segments, and their output is an information content entity instance. For example an investigation aiming to assess the gender of an individual takes different tokens on particular bones, and define the set of expressions.

\img{\IV}{glabExpressions.png}{Glabella and its expressions}{glab}

 \figurename{glab} illustrates the bone segment glabella and its expressions. For the gender estimation multiple assays are defined each with some output values. After the carrying out the assays from the output values the  

The task of the interface is allow the users to perform all the assays by selecting the already created bones and setting the output value. The interface can be seen on \figref{SDE_UI}.
