...
\figref{owlVocab} shows a subset of the OWL vocabulary, that is relevant for the thesis. There are two new class the \textit{owl:Class} and \textit{owl:Restriction}, and two new properties \textit{owl:ObjectProperty} and \textit{owl:DataProperty}. The further subproperties are discussed later through the examples.


\imgK{\II}{15_owl_vocab.png}{A subset of OWL vocabulary}{owlVocab}


For us the most important are the restrictions. 


\imgK{\II}{restrictionScheme.png}{Scheme of the restrictions}{restScheme}


For us the most important feature of OWL are the restrictions. With a restrictions it is possible to express further constraints on the RDF data, above the range and domain definitions. To illustrate the utility of restriction the ontology from the previous section is extended with three further subclasses (\figref{ontExt})

\img{\II}{ontExt1.png}{Ontology extension}{ontExt1}


The abbreviations \textit{CS} stands for computer science. With OWL restrictions it is possible to express for example the rules that a computer science student is allowed to attend only computer science course, and has to take at least two key courses.

Restrictions are conventionally expressed by blank nodes. Blank nodes do not have IRIs, so they are referred as well as anonymus resources.
