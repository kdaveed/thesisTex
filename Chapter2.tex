...
In order to navigate between the pages of the application, the HTML document contains links that trigger further HTTP requests. Links in HTML can be defined by means of the \textit{<a/>} tag. The most important parameter of this tag is \textit{href}, whose value contains the URI of the HTTP request they initiate. In listing \ref{exHTML} an example HTML document is illustrated.
...
The request initiated by the link with the text \text{Page 1} arrives to the same server, and the application processes it return an other HTML page.
