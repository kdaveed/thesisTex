...
In Resource Description Framework (RDF), the information of the web is represented by means of triples. Each triple consists of a subject, predicate and object. The set of triples constitute to an RDF graph, where the subject and object of the triples are the nodes, the predicates are the edges of the graph.
...
Moreover the literal have as well types, but it is expressed.
...
The RDF data has a scheme. The scheme defines how the graph can be built from the resources an literals. Here the most important to introduce the concept of classes. Each RDF resource must belong to class, otherwise it does not have a meaning.

\begin{lstlisting}[basicstyle=\footnotesize, captionpos=b, caption=Instances of classes, label=lst:sparql,
frame=single]
@prefix rdf:   <http://www.w3.org/1999/02/22-rdf-syntax-ns\#>
eq:Bob						rdf:type			eq:Student .
eq:University				rdf:type			eq:University .
\end{lstlisting}
...
eq:Student					rdf:type				rdfs:Class .
eq:University				rdf:type				rdfs:Class	.
eq:attends					rdf:type				rdf:Property .
eq:attends					rdfs:domain				eq:Student	.
eq:attends					rdfs:range				eq:University .
\end{lstlisting}
...
The resources and literals from the previous section have as well types. In the case of resources 
