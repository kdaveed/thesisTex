...
The class \textit{Bone Organ} is the superclass of all bones in the human skeleton. Each bone belongs to a skeletal subdivision and a skeletal subdivision can be a part of another skeletal subdivision. This relationship in both cases is expressed by the property \textit{fma:systemic\_part\_of}. To define which bone organ belongs to which skeletal subdivision FMA uses \textit{owl:someValuesFrom} restrictions (Listing \ref{fmaOWL}). 

\begin{lstlisting}[captionpos=b, caption=Rules of human body expressed in OWL, label=fmaOWL, belowskip=1em, aboveskip=2em,
basicstyle=\footnotesize,frame=single]
...
\figref{fma} shows the main structure of FMA by depicting the restrictions on the property \textit{fma:systemic\_part\_of} with red arrows. These restrictions mean that if a bone organ instance have created, then there must be it skeletal subdivision instance, to that it belongs. 
