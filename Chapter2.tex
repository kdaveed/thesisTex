...
Ontologies are used to describe types, relationships and properties of objects of a certain domain. It is a common practice to use existing ontologies rather than developing them by ourselves. The main reason for this lies in the fact that the ontology development is a time consuming and tedious process. Two ontologies are taken in the project, one for the human anatomy and one for biomedical investigations. Above the these two, during the project a small ontology (called \textit{RDFBones}) is developed acts like a connector. Firstly the two applied ontologies will be discussed and lastly the \textit{RDFBones} ontology.
...
\img{\II}{rdfbones.png}{Extending OBI ontology}{rdfbones}
