...
In the macro definition it can be seen that the url of the link contains the parameter \textit{id}. Based on the mapping the server decides which routine has to be called. The point is that the value of the \textit{id} parameter is substituted into the query that return the grades of the student. In this way can be achieved that the links points to other pages displaying information individually about the students. This is the fundamental mechnaisms how data can be discovered in web pages.
