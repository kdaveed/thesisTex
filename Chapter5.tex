...
The set of triples describing the data model builds a graph where the various input nodes are connected to each other. Further task of the processor algorithm to determine the subgraphs that define the SPARQL queries for the variable that appear on the form. The elements on the form has a specified order, and the rule that have to be considered during the processing, is that a variable can be dependent only of the main input variables or such variables that are before it on the form. The reason is that the dependency means that the SPARQL query has one or more inputs, whose values are substituted into the variables, that constraints the output.

\img{\V}{form.png}{Form element order}{form}

\figref{form} shows the simplified form layout of the skeletal inventories and emphasises the order of the elements with the number. The idea is that values of the selector for variable \textit{subSubdivision} can be only dependent on the main input nodes because it is the first node. The \textit{boneOrgan} in turn can be dependent on the \textit{subSubDivision} too because its value has to be set before the options of the subform is loaded, thus it can be substituted into a SPARQL query.
