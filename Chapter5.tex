...
The aim of this chapter is to present the main mechanism in of the implemented software framework, which is capable of operating on high-level configuration data. Section \ref{51} is more abstract description ob the functionality, while section \ref{52} goes into the implementation details both of the server and client code. By both case the mechanisms are illustrated on the examples of the previous section.


\section{Main software modules and tasks} \label{51}


In \figref{332_1} we have seen the main work flow and components of the framework. \figref{scheme_1} is in turn a more detailed depiction of the components.
 
 
\img{\V}{scheme_1.png}{More detailed scheme}{scheme_1}


As it was described in the previous chapter the whole process starts from the pages for data display with the HTTP requests initiated by the links of the data fields. This section is divided into three components. First is the part (section \ref{511}) discusses the  processing of the configuration data and generation of the functionality descriptor objects for the client and the server. The second part (section \ref{522}) is about the client functionality including the asynchronous communication for asking form data. Finally the third part covers the process after the submission, namely how the RDF data is stored in the database.
...
\img{\V}{processor_tasks.png}{More detailed scheme}{processor_tasks}
