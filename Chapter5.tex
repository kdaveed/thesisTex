...
As it was mentioned in the previous chapter the process of the form loading starts from the profile pages of VIVO. In VIVO the entry form loading is initiated by the property fields, where based on the property IRI the generator class is retrieved from the configuration triples (\figref{fauxRDF}). The generator class returns an instance of \textit{EditConfigurationVTwo} class. The approach implemented in the frame of the thesis does not replace this scheme, but makes it simpler. The idea is that the complete definition is stored in one object called \textit{FormConfiguration} (listing \ref{dataDefinition}).
