...
Above the nodes, the whole graph itself have to be investigated too. In the previous chapter the triple type \textit{MultiTriple} (double line on the figure) were introduced. The rule regarding this type of triple that it divides the whole graph into subgraphs, and the subgraphs are allowed to be connected to each other only by these triples. \figref{valid_2} illustrates the valid and invalid graph arrangements. The reason is that only to this hierarchical graph scheme can the form JSON object be mapped.
