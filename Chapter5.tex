...
<someProperty>  vivo:customEntryFormAnnot 
											"rdfbones.DrawingConclusionGenerator.java"
\end{lstlisting}

The approach implemented in the frame does not replace this structure, but makes it simpler. In our cases we use as well generator classes, but the definition of the data is much simpler and we apply only a generic ftl template file that is used for all data entry form.
...
\begin{lstlisting}[basicstyle=\footnotesize, frame=single, caption={AJAX request server routine}, label=JS_subFormRoutine, captionpos=b, belowskip=1em, aboveskip=2em]
...
\img{\V}{UML_wd.png}{UML class diagram for VariableDependency}{UML_wd}
