...
Data creation about research processes happens through multiple different web pages, where each page is responsible for a particular subset of data. So the general scheme of the input forms is characterized by the subset of the core ontology, while its elements of are defined in the extensions. The idea of the thesis is develop a web application framework that is capable of generating its interfaces based on a declarative definition of the dataset they supposed to create. To achieve this a descriptor language is designed which is capable of expressing the dataset and its mapping to the interface. The utility of the idea is that the developed system can be applied not only for the concise solution of the problems of the \textit{RDFBones} project without coding, but for any kind domains, where the rules are defined through ontology extensions.
