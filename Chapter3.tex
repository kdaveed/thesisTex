...
The previous section showed the basics of forms for multi dimensional data input. An important issue which was just partly addressed in \ref{233}, is how the options of the particular selector fields are loaded. In order to load the options of the initial selector can template files be used, that gets the list of the classes coming from a SPARQL query on the ontology. But there are elements, for example the second selector field in \figref{332_1}, whose values are dependent on the previous elements. This means their option can be loaded dynamically only after selector of it ancestor element.

\img{\III}{322_1.png}{Initial state of the form}{322_2}


Thus a further task of the client code is to be aware of the dependent selector elements and set the appropriate values of them. This problem requires first of all again a data structure about the ontology on the client. A solution could be the following JSON object, which stores the list of the skeletal sub subdivisions grouped by the IRI of the skeletal subdivision. Thus after the selection the program could insert the appropriate list and set to the second selector of the form.
